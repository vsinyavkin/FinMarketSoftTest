...
с HMAC-подписью ходит в TickTrader Web API и показывает котировки, информацию о счёте,
(в задел — Level2 и ордера). Тестовое задание для Finmarket Soft.
...
- `POST /api/orders`, `DELETE /api/orders/{id}?amount=` — каркас (см. «что не успел»)
...
- **Ордера (шаг 7)** — backend-эндпоинты `POST/DELETE /api/orders` сделаны каркасом
  (тонкий проброс, без расширенной валидации и без UI). Level2 проксируется, но UI
  выбора символа/глубины не доделан.
- **Живой прогон к demo-API** не выполнялся — на момент разработки не было реальных
  Web API Id/Key/Secret. Поэтому имена полей DTO/ответов (precision, поля счёта) на фронте
  читаются толерантно и подлежат выверке по реальным образцам JSON.
